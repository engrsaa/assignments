...
let gstnames:string[]=['Kumail','Masood','Muneer'];
for(let i=0;i<gstnames.length;i++){
    console.log(`Hello ${gstnames[i]}, please join us for dinner tommorrow.`);
}
let forQuestion19:string[]=[...gstnames];
export{forQuestion19};
// Question # 15
...
let absentToBe="Masood";
console.log(`${absentToBe} can't make it to dinner.`);
gstnames[gstnames.indexOf(absentToBe)]="Arshad";//in indexOf function we may use array value like "Kumail"
gstnames.forEach(mygsts =>{
    console.log(`Dear ${mygsts}, would you like to join me for dinner?`);
});
...
for(let i=0;i<gstnames.length;i++){
    console.log(`Dear ${gstnames[i]}, please join us for dinner on Saturday.`);
}
...
for(let i=gstnames.length-1;i>1;i--){
    console.log(`Sorry ${gstnames[i]}, I can't invite you for dinner.`);
    gstnames.pop();
}
for(let i=0;i<gstnames.length;i++){
    console.log(`Hello ${gstnames[i]}, you are still invited for dinner on Saturday.`);
}
